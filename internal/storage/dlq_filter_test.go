...
func TestDLQ_ListFiltered_PipelineExactMatch(t *testing.T) {
	...
	_ = s.Pipelines.Create(ctx, p1)
	_ = s.Pipelines.Create(ctx, p2)

	_ = s.DLQ.Insert(ctx, &DLQEntry{PipelineID: p1.ID, OriginalMsg: []byte("a"), ErrorReason: "x"})
	_ = s.DLQ.Insert(ctx, &DLQEntry{PipelineID: p1.ID, OriginalMsg: []byte("b"), ErrorReason: "y"})
	_ = s.DLQ.Insert(ctx, &DLQEntry{PipelineID: p2.ID, OriginalMsg: []byte("c"), ErrorReason: "z"})

	list, total, err := s.DLQ.ListFiltered(ctx, DLQFilter{PipelineID: p1.ID}, 1, 10)
	if err != nil {
		t.Fatal(err)
	}
...
func TestDLQ_ListFiltered_ErrorContainsCaseInsensitive(t *testing.T) {
	...
	_ = s.DLQ.Insert(ctx, &DLQEntry{OriginalMsg: []byte("a"), ErrorReason: "Connection refused"})
	_ = s.DLQ.Insert(ctx, &DLQEntry{OriginalMsg: []byte("b"), ErrorReason: "validation failed"})
	_ = s.DLQ.Insert(ctx, &DLQEntry{OriginalMsg: []byte("c"), ErrorReason: "TLS handshake"})

	list, total, _ := s.DLQ.ListFiltered(ctx, DLQFilter{Error: "connection"}, 1, 10)
	if total != 1 || list[0].ErrorReason != "Connection refused" {
		t.Errorf("case-insensitive contains failed: total=%d items=%+v", total, list)
	}

	list, total, _ = s.DLQ.ListFiltered(ctx, DLQFilter{Error: "FAILED"}, 1, 10)
	if total != 1 {
...
func TestDLQ_ListFiltered_TimeRange(t *testing.T) {
	...
	_ = s.DLQ.Insert(ctx, old)
	_, _ = s.DB.ExecContext(ctx, `UPDATE dlq SET created_at = ? WHERE id = ?`,
		time.Now().Add(-2*time.Hour).UTC(), old.ID)

	_ = s.DLQ.Insert(ctx, &DLQEntry{OriginalMsg: []byte("recent"), ErrorReason: "y"})

	cutoff := time.Now().Add(-time.Hour).UTC()
	list, total, _ := s.DLQ.ListFiltered(ctx, DLQFilter{Since: &cutoff}, 1, 10)
	if total != 1 {
...
func TestDLQ_ListFiltered_CombinedFilters(t *testing.T) {
	...
	_ = s.DLQ.Insert(ctx, &DLQEntry{PipelineID: p.ID, OriginalMsg: []byte("a"), ErrorReason: "validation: bad type"})
	_ = s.DLQ.Insert(ctx, &DLQEntry{PipelineID: p.ID, OriginalMsg: []byte("b"), ErrorReason: "connection reset"})
	_ = s.DLQ.Insert(ctx, &DLQEntry{OriginalMsg: []byte("c"), ErrorReason: "validation: missing field"}) // no pipeline

	list, total, _ := s.DLQ.ListFiltered(ctx, DLQFilter{
		PipelineID: p.ID,
		Error:      "validation",
	}, 1, 10)
...
func TestDLQ_ListFiltered_PaginationCountReflectsFilter(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		_ = s.DLQ.Insert(ctx, &DLQEntry{OriginalMsg: []byte("matching"), ErrorReason: "boom"})
	}
	for i := 0; i < 7; i++ {
		_ = s.DLQ.Insert(ctx, &DLQEntry{OriginalMsg: []byte("other"), ErrorReason: "quiet"})
	}

	list, total, _ := s.DLQ.ListFiltered(ctx, DLQFilter{Error: "boom"}, 1, 3)
	if total != 5 {
