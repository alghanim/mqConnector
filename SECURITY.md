...
- **Session inactivity timeout**: `auth.idle_timeout` (sliding). When set, every authenticated request re-issues the session cookie with `MaxAge = IdleTimeout`. An abandoned browser tab past the idleness budget loses its cookie and the next request lands on the unauthenticated path. Required by HIPAA / NIST 800-53 IA-11 / some PCI flows. Default 0 (disabled) preserves legacy behaviour. See `internal/auth/auth.go` (`cookieMaxAgeSeconds`, `SlideCookie`) and `internal/auth/middleware.go`.
